...
export default function CategoryCard({ apiData, category }) {
  ...
  return (
    ...
    >
      <section class="container mx-auto p-10 md:py-12 px-0 md:p-8 md:px-0">
        <section class="p-5 md:p-0 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-5 gap-y-9 items-start ">
          {apiData.map((data) => {
            return (
              <section
                key={data.id}
                class="relative overflow-hidden category_card rounded-xl p-5 py-10  text-center transform duration-500 hover:-translate-y-2 cursor-pointer"
              >
                <img src={data.image} alt="" class="w-full h-80 object-cover" />
                ...
                <h1 class="text-3xl my-5">{data.name}</h1>
                {/* <p class="mb-5">
                Lorem ipsum dolor sit amet consectetur adipisicing elit.
                Distinctio, incidunt!
              </p> */}
                <h2 class="font-semibold mb-5 ">
                  <span className="text-red-500">{data.price}</span> so'm
                </h2>
